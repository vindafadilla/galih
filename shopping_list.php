<?php
  ...
  if(!isset($_SESSION['barang']))
  {
    ...
    $ada = false;
          ...
          $jumlah_barang = count($_SESSION['barang']);

          for($i = 0; $i < count($_SESSION['barang']); $i++)
          {
            if($_SESSION['barang'][$i]['id'] == $data['id_barang'])
            {
              $ada = true;
              break;
            }
      ...
      if($ada){
               $_SESSION['barang'][$i]['jumlahBeli'] += 1;
      }else{
              $barang = array();
              $barang['id'] = $data['id_barang'];
              $barang['namaBarang'] = $data['nama_barang'];
              $barang['hargaBarang'] = $data['harga'];
              $barang['jumlahBeli'] = 1;
              $barang['totalHargaBeli'] = $data['harga'] * $barang['jumlahBeli'];

              $_SESSION['barang'][$jumlah_barang] = $barang;
            }
